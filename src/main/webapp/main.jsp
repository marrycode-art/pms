...
<script type="text/javascript">
	$(function () {
		$.ajax({
			type:"get",
			url:"${pageContext.request.contextPath}/notice/latestNotice",
			success:function (entity) {
				if (entity.map.statusCode == 200) {
				    $(entity.map.notices).each(function (index,item) {
                    	var li = "<li onclick='showWindow("+ item.nid +")' class='ue-clear'>"
	...
	function showWindow(nid) {
	    $.ajax({
			type:"get",
			url:"${pageContext.request.contextPath}/notice/showInfo/" + nid,
			success:function (entity) {
				if (entity.map.statusCode == 200) {
				    $("#ntitle").text(entity.map.notice.ntitle);
                    $("#content").text(entity.map.notice.remark).css({"font-size":"20px"});
                    $("#showdiv").show();
                    $("#cover").css({"height":document.body.scrollHeight+"px",});
                    $("#cover").show();
                }else {
				    alert(entity.map.message);
                }
            }
		});
	}
...
<div id="showdiv" style="width: 60%; margin: 0 auto; height:500px; border: 1px solid #999; display: none; position: absolute; top: 20%; left: 20%; z-index: 3; background: #fff">
	...
	<!-- 内容 -->
	<div id="content" style="text-indent: 50px; height: 4rem; font-size: 0.5rem; padding: 0.5rem; line-height: 2rem; ">

	</div>
	<!-- 按钮 -->
	<div onmouseover="this.style.color='red'" onmouseout="this.style.color='white'" style="background: green; width: 10%; margin: 0 auto; height: 1.5rem; line-height: 1.5rem; text-align: center;color: #fff;margin-top: 28rem; -moz-border-radius: .128rem; -webkit-border-radius: .128rem; border-radius: .128rem;font-size: .59733rem;" onclick="closeWindow()">
		关闭
	</div>
</div>
